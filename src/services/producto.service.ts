...
export class ProductoService {
  ...
  public async crearProducto(dto: CrearProductoDTO): Promise<ProductoResponseDTO> {
    ...
    const creado = await this.productoRepo.crear(nuevoProducto);
    return this.mapToResponse(creado);
  }

  public async obtenerTodos(): Promise<ProductoResponseDTO[]> {
    const productos = await this.productoRepo.obtenerTodos();
    return productos.map((p) => this.mapToResponse(p));
  }

  public async obtenerPorId(id: number): Promise<ProductoResponseDTO> {
    if (isNaN(id) || id <= 0) {
  ...
  public async actualizarProducto(
    ...
    const existente = await this.productoRepo.obtenerPorId(id);
    if (!existente) {
      throw new NotFoundError(`Producto con ID ${id} no encontrado`);
    }
    ...
    const actualizado = await this.productoRepo.actualizar(id, {
  ...
  public async eliminarProducto(id: number): Promise<boolean> {
    ...
    return await this.productoRepo.eliminar(id);
  }
